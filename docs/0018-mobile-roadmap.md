...
> 核查日期：2026-09-07。本文记录当前产品最终形态、P0/P1 交付结果和后续待办。产品行为以
> [SPEC](./SPEC.md) 为准。
...
- Add Sheet 保留“图片编辑 · 即将推出”入口，当前展示明确的占位说明，不发起编辑请求。
...
P0 数据模型使用开发期迁移脚本重建壁纸表和类型表，数据库结构以当前 Prisma schema 为准。

## 后续待办

### P1：主流程闭环（代码与自动化完成，真机验收本轮不执行）

- **C02 风格选择约束**：共享账号风格查询；首次加载自动选择有效风格，无风格、加载中或请求失败时禁用生成。高级选项默认折叠，保留芯片与画质控件。
- **C03 任务恢复**：新增授权 `GET /jobs`（全部未完成任务）与
...
- **C04 归档反馈**：结果页显示已归档类型，提供查看作品、查看类型、创建新作品入口；恢复的任务缺少本地原始参数时不提供无效的“重新生成”。
- **U01/U02 反馈完整性**：封面、网格、结果和全屏预览共用图片失败重试组件，重试时刷新访问地址；收藏提交中禁用重复操作，失败显示错误并允许重试。
- **N01/N02 平台操作与清理**：Android 桌面/锁屏/两者、相册权限、保存、分享及失败重试已覆盖 mock 测试。下载文件使用应用专用临时目录，成功、失败及迟到下载均清理；启动和账号切换清理该目录，系统操作前复核会话。iOS 提供保存/分享和手动设置说明，Web 显示能力说明并隐藏不支持的原生操作。**用户明确要求本轮不做真机验收，系统效果与真实权限行为未验证。**
- **Q01 真实质量展示**：存储层从上传图像字节读取实际宽高，归档时优先采用该尺寸；SiliconFlow 不再把请求尺寸当成实际结果返回。结果页显示像素尺寸，未知时显示“尺寸不可用”，移除固定“Full
  2K+”承诺。

P1 同时修复生成、收藏和创建类型的迟到响应：每次操作记录账号会话版本，退出再登录同一账号也使旧版本失效；API 请求在账号变化时中止。回归测试覆盖挂载中的 hooks 切换账号、创建类型后切换账号、下载完成前切换账号。

任务恢复指 **Mobile 应用重启**
后重新查询已落库任务，不包含服务端进程重启后的持久化 worker 重调度。自动化通过不代表真实 Clerk、Provider、R2 的端到端验收完成。此前 Review 中的源图归属与 URL/object
key 混用问题仍需在编辑管线中修复，不能据此宣称完整图片编辑已完成。

本轮验证：服务端 13 个测试文件 / 44 项测试，Mobile 25 个测试套 / 61 项测试；workspace
check、服务端编译及 Android/iOS/Web 导出通过。Mobile Jest 使用
`--forceExit`，未将自然退出作为通过项。

### P2：完整图片编辑与体验

- **E02 图片编辑**：选图、授权上传、编辑、扩图、增强、提取风格、失败重试、结果归档和私有风格复用。
- **U03 大列表性能**：类型详情改用虚拟化列表、缩略图和合理缓存，覆盖 200 张以上作品。
- **U04 国际化与无障碍**：补齐类型、账号错误、占位入口和操作状态文案，验证读屏、焦点和大字号布局。
- **U05 平台一致性**：Web 提供可用下载和平台能力说明，原生平台按系统能力展示操作。
