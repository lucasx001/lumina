...
`sourceImageKey`，Provider 只接收服务端按 key 签发的短期 URL；不能据此宣称完整图片编辑已完成。

本轮验证：服务端 13 个测试文件 / 47 项测试，Mobile 25 个测试套 / 63 项测试（使用
`--forceExit`）；服务端编译、Mobile/Landing TypeScript 检查通过。Mobile
Jest 自然退出仍有异步句柄；完整 `vp check`
受仓库既有格式差异影响，Android/iOS/Web 导出未在本轮重复执行。
