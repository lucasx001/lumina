...
异步生成、轮询、预设、作品列表与收藏已有接口。编辑请求只接受账号归属的
`sourceImageKey`，不再接受任意远程 URL；结果图可通过受保护的 `/wallpapers/:id/image`
重新获取短期凭据。目标账号授权、类型目录和独立详情见 A01–A05、D01–D03。

## 验收标准

- [x] A/B 对所有读写接口的越权测试通过（源图 key、任务、作品详情和结果图读取均按账号校验）。
