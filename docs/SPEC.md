...
6. 图片编辑入口可见、可达，支持授权上传、编辑、扩图、增强和提取私有风格；结果归属于当前账号。
...
Add
Sheet 提供次级图片编辑入口。用户可从相册选择 JPEG、PNG 或 WebP 图片并通过账号授权上传，然后选择编辑、扩图、增强或提取风格；生成结果保存到选定类型，提取的风格保存为当前账号的私有预设。Provider 不支持的模式必须显示可重试的错误，不展示虚假的成功状态。
...
优先完成账号边界和完整数据访问，再完成创建归档、图片编辑和真机操作闭环。详细排序与代码证据见
